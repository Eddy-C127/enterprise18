...
class ResConfigSettings(models.TransientModel):
    ...
    intercompany_receipt_type_id = fields.Many2one(related='company_id.intercompany_receipt_type_id', readonly=False,
                                      domain=lambda self: [('warehouse_id.company_id', '=', self.env.company.id), ('code', '=', 'incoming')])
    sync_delivery_receipt = fields.Boolean(related='company_id.sync_delivery_receipt', readonly=False)
    ...
    def onchange_rule_type(self):
        if self.rule_type not in new_rule_type.keys():
            ...
            if not self.intercompany_receipt_type_id:
                self.intercompany_receipt_type_id = self.env['stock.picking.type'].search([('code', '=', 'incoming'), ('warehouse_id.company_id', '=', self.env.company.id)], limit=1)
            self.sync_delivery_receipt = self.env.user.has_group('stock.group_production_lot')
