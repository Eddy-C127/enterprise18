    ...
    def _generate_export_file(self):
        ...
        if self.payment_method_code == 'aba_ct':
            ...
            if bank_account.acc_type != 'aba' or not bank_account.aba_bsb:
                raise UserError(_("The account %(account)s, of journal '%(journal)s', is not valid for ABA.\nEither its account number is incorrect or it has no BSB set.", account=bank_account.acc_number, journal=self.journal_id.name))

            if not self.journal_id.aba_fic or not self.journal_id.aba_user_spec or not self.journal_id.aba_user_number:
                raise UserError(_("The account %(account)s, of journal '%(journal)s', is not set up for ABA payments.\nPlease fill in its ABA fields.", account=bank_account.acc_number, journal=self.journal_id.name))
