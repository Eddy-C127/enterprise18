...
class AccountBatchPayment(models.Model):
    ...
    def _validate_bank_for_nacha(self, payment):
        ...
        if not bank.aba_routing:
            raise ValidationError(_("Please set an ABA routing number on the %(account)s bank account for %(partner)s.", account=bank.display_name, partner=payment.partner_id.display_name))
    ...
    def _generate_nacha_batch_header_record(self, date, batch_nr):
        ...
        batch.append(f"{self.journal_id.nacha_discretionary_data or self.name[-20:]:20.20}")  # Company Discretionary Data (optional)

        batch.append(f"{self.journal_id.nacha_company_identification:10.10}")  # Company Identification
