...
class CommissionPlan(models.Model):
    ...
    date_from = fields.Date("From", required=True, default=fields.Date.today, tracking=True)
    date_to = fields.Date("To", required=True, default=lambda self: fields.Date.today() + relativedelta(years=1), tracking=True)
    periodicity = fields.Selection([('month', "Monthly"), ('quarter', "Quarterly"), ('year', "Yearly")], required=True, default='quarter', tracking=True)

    commission_amount = fields.Monetary("On target Commission", help='OTC', currency_field='currency_id')
    ...
    user_ids = fields.One2many('sale.commission.plan.user', 'plan_id')
