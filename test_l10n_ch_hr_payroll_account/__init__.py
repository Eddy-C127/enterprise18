...
def _generate_ch_demo_payslips(env):
    ...
    if env.ref('base.demo_company_ch', raise_if_not_found=False):
        if not env['hr.payslip'].sudo().search_count([('employee_id.name', '=', 'Michael Lambert'), ('company_id', '=', env.ref('base.demo_company_ch').id)]):
            _logger.info('Generating payslips')
            employees = env['hr.employee'].search([
                ('company_id', '=', env.ref('base.demo_company_ch').id),
            ])
            wizard_vals = {
            ...
            cids = env.ref('base.demo_company_ch').ids
